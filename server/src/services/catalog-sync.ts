...
// Local model providers: models seeded by migrations that the remote catalog
// does not know about (Chinese domestic providers, private deployments, etc.).
// Catalog sync never inserts, updates, or deletes rows from these providers —
// they are fully managed locally. This prevents remote catalog wipes from
// removing custom providers that freellmapi.co does not list.
const LOCAL_MODEL_PROVIDERS = new Set<Platform>(['baidu-qianfan', 'aliyun-bailian']);
const LOCAL_MODEL_IDS = new Set(['google:gemini-3.1-flash-lite']);
...
export function applyCatalog(db: Db, catalog: Catalog): NonNullable<SyncResult['counts']> {
  ...
  const apply = db.transaction(() => {
    ...
    for (const m of catalog.models) {
      ...
      // Local-managed providers are never updated by remote catalog — their
      // models are seeded by migrations and configured locally.
      if (LOCAL_MODEL_PROVIDERS.has(m.platform as Platform)) {
    ...
    for (const c of candidates) {
      ...
      if (LOCAL_MODEL_PROVIDERS.has(c.platform as Platform)) continue; // protect local provider models from deletion
      if (LOCAL_MODEL_IDS.has(`${c.platform}:${c.model_id}`)) continue; // protect verified stable Gemini ID
