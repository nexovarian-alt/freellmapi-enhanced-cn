...
let _proxyUrl = '';
let _envProxyUrls: { http: string; https: string } = { http: '', https: '' };
...
export function applyProxyUrl(dbValue: string): void {
  ...
  cached = null;
  if (_proxyUrl || _envProxyUrls.http || _envProxyUrls.https) {
    const masked = _proxyUrl.replace(/\/\/[^@]*@/, '//***@');
    const envNames = [_envProxyUrls.http && 'HTTP_PROXY', _envProxyUrls.https && 'HTTPS_PROXY']
      .filter(Boolean).join(',');
    console.log(`[proxy] Configured ${masked || envNames}`);
  } else {
...
function proxyForUrl(url: string): string {
  if (_proxyUrl) return _proxyUrl;
  try {
    return new URL(url).protocol === 'http:' ? _envProxyUrls.http : _envProxyUrls.https;
  } catch {
    return '';
  }
}
...
async function dispatchFetch(
  ...
  const resolved = await resolveDispatcher(proxyForUrl(url));
...
export function isProxyActive(): boolean {
  if (!_initialized) applyProxyUrl('');
  return _proxyEnabled && !!(_proxyUrl || _envProxyUrls.http || _envProxyUrls.https);
}
